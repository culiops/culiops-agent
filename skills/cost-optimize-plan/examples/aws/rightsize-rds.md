...
2. `aws cloudwatch get-metric-statistics --namespace AWS/RDS --metric-name CPUUtilization --dimensions Name=DBInstanceIdentifier,Value=<id> --start-time <now-30d> --end-time <now> --period 86400 --statistics Average,Maximum` — 30d CPU.
3. `aws cloudwatch get-metric-statistics --namespace AWS/RDS --metric-name FreeableMemory --dimensions Name=DBInstanceIdentifier,Value=<id> --start-time <now-30d> --end-time <now> --period 86400 --statistics Minimum` — 30d memory headroom (Minimum, because we want worst-case).
4. `aws cloudwatch get-metric-statistics --namespace AWS/RDS --metric-name DatabaseConnections --dimensions Name=DBInstanceIdentifier,Value=<id> --start-time <now-30d> --end-time <now> --period 86400 --statistics Maximum` — peak connection count.
...
| 30d peak CPU | < 75% on new class | ≥ 90% on new class |
| 30d Minimum FreeableMemory | > 25% of current class's RAM | < 10% of new class's RAM (about to OOM after downsize) |
| 30d peak DatabaseConnections | < 70% of new class's max_connections | ≥ 95% of new class's max_connections |
